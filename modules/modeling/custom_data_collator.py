...
class DataCollatorForMultiTask(DataCollatorForTokenClassification):
    
    def __init__(self, dst_label, eye_gaze_labels, *args, **kwargs):
    ...
    def torch_call(self, features):
        import torch
        all_labels = self.eye_gaze_labels + [self.dst_label]
        
        no_labels_features = [{k: v for k, v in feature.items() if k not in all_labels} for feature in features]
        ...
        all_eye_gaze_labels = dict()
        for label_name in self.eye_gaze_labels:
            all_eye_gaze_labels[label_name] = [feature[label_name] for feature in features]

        if len(all_eye_gaze_labels) == 0:
            return batch
        ...
        if padding_side == "right":
            for label_name in self.eye_gaze_labels:
                labels = all_eye_gaze_labels[label_name]
                batch_labels_dict[label_name] = [
                    to_list(label) + [self.label_pad_token_id] * (sequence_length - len(label)) for label in labels
                ]
        else:
        ...
        for label in self.eye_gaze_labels:
            batch[label] = torch.tensor(batch_labels_dict[label_name], dtype=torch.float32)
        
        
        batch[self.dst_label] =  torch.tensor([f[self.dst_label] for f in features], dtype=torch.float32)
