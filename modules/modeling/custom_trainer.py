...
class InterleavedMultitaskFinetuningTrainer(NoShuffleTrainer):
    def prediction_step(
        self,
        model: nn.Module,
        ...
        """
        Override prediction step to predict only the labels of the considered task (otherwise has_labels would be set to False)
        """
        has_labels = False if len(self.label_names) == 0 else any(inputs.get(k) is not None for k in self.label_names)
        active_labels = [k for k in self.label_names if inputs.get(k) is not None]
        ...
        loss_without_labels = True if len(active_labels) == 0 and return_loss else False
        ...
        if has_labels or loss_without_labels:
            labels = nested_detach(tuple(inputs.get(name) for name in active_labels))
            if len(labels) == 1:
                labels = labels[0]
        else:
            labels = None
    ...
    def evaluation_loop(
        ...
        """
        Used metric_key_prefix to call the task specific compute metric function
        """
        task_name = metric_key_prefix[len('eval_'):]
        args = self.args
        ...
        if (
            ...
            if args.include_inputs_for_metrics:
                ...
                metrics = self.compute_metrics[task_name](EvalPrediction(predictions=all_preds, label_ids=all_labels))
